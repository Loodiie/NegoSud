...
public class WineOrderDAO {
    ...
    public List<CommandesClients> getAllWineOrders() {
        StringBuilder response = new StringBuilder();

        try {
            URL url = new URL("http://localhost:29200/api/v1/commandesClients");
    ...
    private List<CommandesClients> parseJsonArray(String jsonArray) {
        try {
            ObjectMapper objectMapper = new ObjectMapper();
            return objectMapper.readValue(jsonArray, new TypeReference<List<CommandesClients>>() {});
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public CommandesClients getWineOrderById(int id) {
        StringBuilder response = new StringBuilder();
    ...
    private CommandesClients parseJsonString(String jsonString) {
        try { // Utilisation de Jackson ObjectMapper pour mapper la chaîne JSON vers un objet Java
            ObjectMapper objectMapper = new ObjectMapper();
            return objectMapper.readValue(jsonString, CommandesClients.class);
        } catch (Exception e) {
            e.printStackTrace();
            // TODO Gérer les erreurs de parsing ici
            return null;
